...
\begin{tikzpicture}[>=stealth',shorten >=1pt,auto,node distance=2.8cm,
  ...
  \path (B) edge              node {5} (C)
        ...
        (C) edge              node {4} (D)
            ...
            edge              node {1} (A);
\end{tikzpicture}
\caption{\footnotesize{El recorrido representado no es óptimo. Si bien tiene todos los caminos mínimos también tiene uno de mas, el camino directo del vertice 0 al 1, este agrega un costo de 10 el cual es innecesario debida a que de todas maneras ya estabamos visitando con costo 6 al planeta 1.}}
\end{minipage}
\hfill
\begin{minipage}[b]{0.4\textwidth}
\centering
